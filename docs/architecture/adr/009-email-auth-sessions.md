...
- Cập nhật UX `MOB-P1-001` (2026-09-22): thêm `POST /auth/email-step` nhận email và trả duy nhất `nextStep: login|register`, `Cache-Control: no-store`, cùng rate guard. Đây là ngoại lệ có chủ đích cho email-first: cho phép biết email đã có account, không trả profile/password/verification state. Legacy account cũng vào login; discovery không tạo hay thay credential. Register vẫn tự gửi verify email; mobile chuyển sang nhập mã, không có nút verify/reset độc lập ở màn đầu.
...
Không thêm Redis runtime hay JWT signing/refresh rotation; đổi lại mọi auth request phụ thuộc DB. Khóa user đảm bảo reset/session consistency. Rate limit email có thể bị lợi dụng làm user phải chờ window; cửa sổ hữu hạn, không lock tài khoản vĩnh viễn. Register/request email giữ response thống nhất, nhưng `email-step` cố ý tiết lộ account existence để phân nhánh UX. Rate limit không loại bỏ enumeration; timing/traffic analysis và abuse quy mô lớn vẫn cần review `SEC-P1-001`, cùng edge rate limit cho payload malformed trước controller.
